...
func run_tests():
	...
	var options = ["-s", "res://addons/aerobeat-vendor-godot-unit-test/gut_cmdln.gd", "-graie", "-gdisable_colors",
...
func get_gut_help():
	_text_buffer = ''
	var options = ["-s", "res://addons/aerobeat-vendor-godot-unit-test/gut_cmdln.gd", "-gh", "--headless"]
	await _run_blocking(options)
	return _text_buffer
